...
@Service
public class GetFilteredEmployeesService implements GetFilteredEmployeesUseCase {

    private static final Logger log = LoggerFactory.getLogger(GetFilteredEmployeesService.class);
    ...
    public GetFilteredEmployeesService(
    ...
    @Override
    public Page<EmployeeWithDependentsResponse> execute(EmployeeFilter filter, Pageable pageable) {
        return repository.findByFilter(filter.getCpf(), filter.getBirthDate(), pageable).map(view -> {
    ...
    private List<Dependent> parseDependents(String json) {
        ...
        try {
            ...
            log.warn("Failed to parse dependents JSON: {}", e.getMessage());
            return Collections.emptyList();
        }
    }
}
