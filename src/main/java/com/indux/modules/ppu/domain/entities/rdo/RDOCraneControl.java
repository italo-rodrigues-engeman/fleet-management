package com.indux.modules.ppu.domain.entities.rdo;

import com.fasterxml.jackson.annotation.JsonProperty;
import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.NoArgsConstructor;

@Data
@AllArgsConstructor
@NoArgsConstructor
public class RDOCraneControl {
    @JsonProperty("operacional")
    private boolean operational;
    @JsonProperty("horimetro")
    private String hourMeter;
    @JsonProperty("checklistOperador")
    private boolean operatorChecklist;
    @JsonProperty("checklistSupervisor")
    private boolean supervisorChecklist;
    @JsonProperty("responsabilidadeInoperancia")
    private String inoperabilityResponsibility;
    @JsonProperty("justificativa")
    private String justification;
}


